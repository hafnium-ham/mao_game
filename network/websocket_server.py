...
class WebSocketGameServer:
    ...
    def __init__(self, host: str = "0.0.0.0", port: int = 8080,
        ...
        self.mao_timer: Optional[asyncio.TimerHandle] = None
    ...
    async def _handle_leave(self, message: Message, player_id: str):
        ...
        async with self.lock:
            if self.game and self.game.remove_player(player_id):
        ...
        async with self.lock:
            ...
            if self.game.start_game():
                ...
                for player in self.game.players:
                    await self._send_hand_update(player.id)
    ...
    async def _handle_play_card(self, message: Message, player_id: str):
        ...
        async with self.lock:
            if not self.game or self.game.phase != GamePhase.IN_PROGRESS:
            ...
            if self.game.play_card(player_id, card):
                ...
                await self._send_hand_update(player_id)
    ...
    async def _handle_throw(self, message: Message, player_id: str):
        ...
        async with self.lock:
            ...
            if self.game.throw_card(player_id, card, target_id):
                ...
                await self._send_hand_update(player_id)
                await self._send_hand_update(target_id)
    ...
    async def _handle_penalty(self, message: Message, player_id: str):
        ...
        async with self.lock:
            ...
            if penalty:
                ...
                target_ws = self.clients.get(target_id, {}).get('ws')
                if target_ws:
                ...
                if return_card:
                    ...
                    if last_play and last_play[0] == target_id:
                        ...
                        if result:
                            ...
                            await self._send_hand_update(original_id)
                            await self._broadcast(Message(
                                type=MessageType.NOTIFICATION,
    ...
    async def _handle_end_poo(self, message: Message, player_id: str):
        ...
        async with self.lock:
            if not self.game or self.game.phase != GamePhase.POINT_OF_ORDER:
        ...
        async with self.lock:
            ...
            if self.game.add_vote(player_id, vote):
                ...
                if result is not None:
                    await self._resolve_penalty_vote(result)

    async def _resolve_penalty_vote(self, result: str):
        ...
        if result == "overturn":
            ...
            await self._send_hand_update(penalty_info["target_id"])
            await self._send_hand_update(penalty_info["caller_id"])
        else:
    ...
    async def _handle_declare_mao(self, message: Message, player_id: str):
        ...
        async with self.lock:
            ...
            if self.game.start_mao_declaration(player_id):
                ...
                self.mao_timer = loop.call_later(MAO_CHALLENGE_TIME,
                    lambda: asyncio.create_task(self._mao_timer_expired(player_id)))

                print(f"{player.name} declared Mao - {MAO_CHALLENGE_TIME} second timer started")

    async def _mao_timer_expired(self, player_id: str):
    ...
    async def _handle_cancel_mao(self, message: Message, player_id: str):
        ...
        async with self.lock:
            if not self.game or self.game.mao_declaring_player_id is None:
    ...
    async def _send_hand_update(self, player_id: str):
    ...
    async def _send_player_list(self, player_id: str):
    ...
    async def _broadcast(self, message: Message, exclude: Optional[Set[str]] = None):
